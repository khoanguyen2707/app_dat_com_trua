...
văn phòng`,
  ...
  me: {
    ...
    viewMenu: (n: number) => `Thực đơn hôm nay (${n} món)`,
    restOfWeek: "Còn lại trong tuần",
    weekTotal: "Cả tuần",
    dayEmptyPast: "Không đặt",
    dayEmptyToday: "Chưa đặt",
    dayFuture: "Chưa tới ngày",
    drinkCount: (n: number) => `${n} đồ uống`,

    payTitle: "Thanh toán của tôi",
    payNothing: "Bạn không còn khoản nào phải trả. Cảm ơn bạn!",
    debtTotal: "Tổng còn nợ",
    debtWeeks: (n: number) => `${n} tuần chưa thanh toán`,
    payAll: "Thanh toán tất cả",
    payWeek: "Thanh toán tuần này",
    payingAll: (n: number) => `Thanh toán gộp ${n} tuần`,
    payingWeek: (label: string) => `Thanh toán tuần ${label}`,
    reportAllBtn: "Tôi đã chuyển khoản tất cả",
    pendingAmount: (money: string) => `${money} đang chờ admin xác nhận`,
  ...
  grid: {
    ...
    guide: {
      title: "Cách đặt cơm",
      order: (price: string) =>
        `Chạm ô để đặt cơm (${price}/ngày) — bắt buộc chọn ít nhất 1 món.`,
      detail: (where: string) =>
      ...
      whereDesktop: "ô",
      today: (cutoff: string) =>
        `Chỉ đặt cho HÔM NAY, trước ${cutoff} — không đặt trước cho ngày sau.`,
      cancel:
        "Nút × (mobile: nút tick) dọn ô: có cơm → bỏ cơm, giữ nước (ô xanh); ô chỉ nước → bỏ nước.",
    ...
    today: {
      ...
      empty: "Hôm nay chưa ai đặt.",
      plainRice: "Cơm",
      copied: "Đã chép danh sách hôm nay",
      notSent: "Chưa gửi cho quán",
      sentAt: (time: string, who: string) => `Đã gửi lúc ${time} · ${who}`,
      markSent: "Đã gửi quán",
      undoSent: "Chưa gửi",
      deadline: (time: string) => `Quán ngừng nhận lúc ${time}`,
      markedSent: "Đã ghi nhận gửi đơn",
      markedUnsent: "Đã bỏ đánh dấu",
    },
    ...
    detail: {
      ...
      noteSection: "Ghi chú cho người đi mua",
      notePlaceholder: "VD: ít cơm, không hành, thêm canh…",
      noteHint: "Người đi mua sẽ đọc dòng này.",
  ...
  menu: {
    ...
    tapToOrder: "chạm vào món để đặt",
    groupOther: "Món khác",
    ...
    post: {
      ...
      intro:
        "Dán thực đơn hôm nay → hệ thống tự nhận diện món (kể cả thiếu dấu/viết tắt), tạo món mới và ẩn món không bán hôm nay.",
      dayLabel: "Ngày bán",
      ...
      groupCreateHint:
        "Món nghi trùng với món đã có phải chọn rõ trước khi áp dụng.",
      useExisting: (name: string) => `Dùng "${name}"`,
      ...
      notifyHint:
        "Gửi thực đơn vừa đăng sang Power Automate để đăng vào nhóm Teams.",
      notifySent: "Đã gửi thông báo Teams",
      notifyFailed: (err: string) =>
        `Đã lưu thực đơn nhưng KHÔNG gửi được Teams: ${err}`,
      nothing: "Thực đơn hôm nay đang trống.",
      summary: (create: number, sell: number, hide: number) =>
        `Tạo ${create} • bán ${sell} • ẩn ${hide}`,
    },
  },
  ...
  payment: {
    ...
    servingsAmount: (servings: number, money: string) =>
      `${servings} suất • ${money}`,
    servings: (n: number) => `${n} suất`,
    ...
    servingsTimesPrice: (servings: number, price: string) =>
      `${servings} suất × ${price}`,
    ...
    debtsTitle: "Công nợ các tuần",
    debtsHint: "Mọi khoản chưa được xác nhận, gộp theo người.",
    debtsEmpty: "Không còn ai nợ. 🎉",
    confirmAll: "Xác nhận tất cả",
    confirmedAllToast: (n: number) => `Đã xác nhận ${n} tuần`,
  },
...
Mật khẩu: ${pw}`,
    ...
    lastAdminHint:
      "Admin duy nhất đang hoạt động — cấp quyền cho người khác trước đã",

    fieldFullName: "Họ tên",
    fieldTeamsEmail: "Email Teams (Microsoft 365)",
    teamsPlaceholder: "ten.ban@congty.com",
    teamsHint:
      "Dùng để @mention đúng người trong Teams khi tới lượt đi lấy cơm. Bỏ trống → Power Automate phải dùng email đăng nhập app, thường KHÔNG mention được.",
